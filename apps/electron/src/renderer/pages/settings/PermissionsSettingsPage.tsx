...
export default function PermissionsSettingsPage() {
  ...
  return (
    <div className="h-full flex flex-col">
      <PanelHeader title={te('Permissions')} actions={<HeaderMenu route={routes.view.settings('permissions')} helpFeature="permissions" />} />
          ...
          <div className="px-5 py-7 max-w-3xl mx-auto">
            <div className="space-y-8">
              {isLoading ? (
                ...
                <>
                  {/* About Section */}
                  <SettingsSection title={te('About Permissions')}>
                    <SettingsCard className="px-4 py-3.5">
                      <div className="text-sm text-muted-foreground leading-relaxed space-y-1.5">
                        <p>
                          {te('Permissions control how much autonomy your agent has. In ')}
                          <span className="text-foreground/80 font-medium">{te('Explore')}</span>
                          {te(' mode, the agent can only read and research — perfect for understanding a problem before committing to changes. When you\'re ready, switch to ')}
                          <span className="text-foreground/80 font-medium">{te('Execute')}</span>
                          {te(' mode to let the agent implement the plan autonomously.')}
                        </p>
                        <p>
                          {te('A good workflow: start in Explore to let the agent investigate, review the proposed plan, then execute with confidence.')}
                        </p>
                        <p>
                          <button
                            type="button"
                  ...
                  <SettingsSection
                    title={te('Default Permissions')}
                    description={te('App-level patterns allowed in Explore mode. Commands not on this list are blocked.')}
                    action={
                    ...
                    <SettingsCard className="p-0">
                      {defaultPermissionsData.length > 0 ? (
                        <PermissionsDataTable
                          ...
                          maxHeight={350}
                          fullscreen
                          fullscreenTitle={te('Default Permissions')}
                        />
                      ) : (
                        <div className="p-8 text-center text-muted-foreground">
                          <p className="text-sm">{te('No default permissions found.')}</p>
                          <p className="text-xs mt-1 text-foreground/40">
                            {te('Default permissions should be at ')}<code className="bg-foreground/5 px-1 rounded">~/.craft-agent/permissions/default.json</code>
                          </p>
                        </div>
                      )}
                    </SettingsCard>
                  ...
                  <SettingsSection
                    title={te('Workspace Customizations')}
                    description={te('Workspace-level patterns that extend the app defaults above.')}
                    action={
                      (() => {
                    ...
                    <SettingsCard className="p-0">
                      {customPermissionsData.length > 0 ? (
                        <PermissionsDataTable
                          ...
                          maxHeight={350}
                          fullscreen
                          fullscreenTitle={te('Workspace Customizations')}
                        />
                      ) : (
                        <div className="p-8 text-center text-muted-foreground">
                          <p className="text-sm">{te('No custom permissions configured.')}</p>
                          <p className="text-xs mt-1 text-foreground/40">
                            {te('Create a ')}<code className="bg-foreground/5 px-1 rounded">permissions.json</code>{te(' file in your workspace to add custom rules.')}
                          </p>
                        </div>
                      )}
                    </SettingsCard>
