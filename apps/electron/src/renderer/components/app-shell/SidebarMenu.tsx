...
export function SidebarMenu({
  ...
  if ((type === 'allSessions' || type === 'status' || type === 'flagged') && onConfigureStatuses) {
    return (
      <MenuItem onClick={onConfigureStatuses}>
        ...
        <span className="flex-1">{te('Configure Statuses')}</span>
      </MenuItem>
    )
  }
      ...
      <>
        {onAddLabel && (
          <MenuItem onClick={() => onAddLabel(labelId)}>
            ...
            <span className="flex-1">{te('Add New Label')}</span>
          </MenuItem>
        )}
        {onConfigureLabels && (
          <MenuItem onClick={() => onConfigureLabels(labelId)}>
            <Settings2 className="h-3.5 w-3.5" />
            <span className="flex-1">{te('Edit Labels')}</span>
          </MenuItem>
        )}
        {labelId && onDeleteLabel && (
          <>
            ...
            <MenuItem onClick={() => onDeleteLabel(labelId)}>
              ...
              <span className="flex-1">{te('Delete Label')}</span>
            </MenuItem>
          </>
        )}
      </>
      ...
      <>
        {onConfigureViews && (
          <MenuItem onClick={onConfigureViews}>
            <Settings2 className="h-3.5 w-3.5" />
            <span className="flex-1">{te('Edit Views')}</span>
          </MenuItem>
        )}
        {viewId && onDeleteView && (
          <>
            ...
            <MenuItem onClick={() => onDeleteView(viewId)}>
              <Trash2 className="h-3.5 w-3.5" />
              <span className="flex-1">{te('Delete View')}</span>
            </MenuItem>
          </>
        )}
      </>
  ...
  if (type === 'sources') {
    ...
    const learnMoreLabel = sourceType === 'api'
      ? te('Learn More about APIs')
      : sourceType === 'mcp'
        ? te('Learn More about MCP')
        : sourceType === 'local'
          ? te('Learn More about Local Folders')
          : te('Learn More about Sources')
      ...
      <>
        {onAddSource && (
          <MenuItem onClick={onAddSource}>
            <Plus className="h-3.5 w-3.5" />
            <span className="flex-1">{te('Add Source')}</span>
          </MenuItem>
        )}
        <Separator />
  ...
  if (type === 'skills' && onAddSkill) {
    return (
      <MenuItem onClick={onAddSkill}>
        <Plus className="h-3.5 w-3.5" />
        <span className="flex-1">{te('Add Skill')}</span>
      </MenuItem>
    )
  }
      ...
      <>
        {onAddAutomation && (
          <MenuItem onClick={onAddAutomation}>
            <Plus className="h-3.5 w-3.5" />
            <span className="flex-1">{te('Add Automation')}</span>
          </MenuItem>
        )}
        <Separator />
        <MenuItem onClick={() => window.electronAPI.openUrl(getDocUrl('automations'))}>
          <ExternalLink className="h-3.5 w-3.5" />
          <span className="flex-1">{te('Learn More about Automations')}</span>
        </MenuItem>
      </>
    )
  }
