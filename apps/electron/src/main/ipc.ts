...
import { appendFile, readFile, readdir, stat, realpath, mkdir, writeFile, unlink } from 'fs/promises'
      ...
      if (attachment.base64) {
        ...
        if (attachment.type === 'image') {
          ...
          if (!validation.valid && validation.errorCode === 'dimension_exceeded') {
            ...
            // File >5MB — try resize+compress instead of rejecting outright
            shouldResize = true
          ...
          if (shouldResize) {
            ...
            if (targetSize) {
              ...
              // Size-exceeded or optimal resize path — shared utility handles re-encode/compress and strips metadata.
  ...
  ipcMain.handle(IPC_CHANNELS.SETUP_LLM_CONNECTION, async (_event, setup: LlmConnectionSetup): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      if (setup.piAuthProvider) {
        ...
        // Update connection name to show the actual provider (e.g. "MoonCake Backend (Google AI Studio)")
        const providerName = piAuthProviderDisplayName(setup.piAuthProvider)
        if (providerName) {
          updates.name = `MoonCake Backend (${providerName})`
        }
