...
export default function InputSettingsPage() {
  ...
  return (
    <div className="h-full flex flex-col">
      <PanelHeader title={te('Input')} actions={<HeaderMenu route={routes.view.settings('input')} />} />
      <div className="flex-1 min-h-0 mask-fade-y">
        <ScrollArea className="h-full">
          <div className="px-5 py-7 max-w-3xl mx-auto">
            <div className="space-y-8">
              {/* Typing Behavior */}
              <SettingsSection title={te('Typing')} description={te('Control how text is entered in the chat input.')}>
                <SettingsCard>
                  <SettingsToggle
                    label={te('Auto capitalisation')}
                    description={te('Automatically capitalise the first letter when typing a message.')}
                    checked={autoCapitalisation}
                    onCheckedChange={handleAutoCapitalisationChange}
                  />
                  <SettingsToggle
                    label={te('Spell check')}
                    description={te('Underline misspelled words while typing.')}
                    checked={spellCheck}
              ...
              {/* Send Behavior */}
              <SettingsSection title={te('Sending')} description={te('Choose how to send messages.')}>
                <SettingsCard>
                  <SettingsMenuSelectRow
                    label={te('Send message with')}
                    description={te('Keyboard shortcut for sending messages')}
                    value={sendMessageKey}
                    onValueChange={handleSendMessageKeyChange}
                    options={[
                      { value: 'enter', label: te('Enter'), description: te('Use Shift+Enter for new lines') },
                      { value: 'cmd-enter', label: isMac ? '⌘ Enter' : 'Ctrl+Enter', description: te('Use Enter for new lines') },
                    ]}
                  />
                </SettingsCard>
              </SettingsSection>
